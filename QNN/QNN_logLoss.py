...
def constructCircuit_QNN(Y,normalized_Xtrain,mf,matrix_feature,theta_test_matrix,n_feature,PARAM,opZ,count,norm_Xtrain_l):
	...
	def cost_exp():
		...
		n_Y = len(Y)
		n_Y_inv = 1/n_Y
		thetas = np.zeros(n_qubit)	#n_feature
		expect_collect = []
		for i in range(n_Y):
			qc_eval = qc.copy()
			datai = np.array(normalized_Xtrain[i])
...
def find_cost_rec_QNN(AA,BB,CC,normalized_Xtrain,Y,component,num_samples_classical):
	...
	def cost_rec(theta):
		...
		'''expect = []
		cost = np.zeros(len(theta))
		for th in range(len(theta)):
			expect.append([])
			for a in range(len(AA)):
				expect[-1].append(((AA[a]*np.cos(theta[th]*normalized_Xtrain[:,component][a]-BB[a])+CC[a])).item())
				cost[th] +=(Y[a]*expect[-1][-1])/len(AA)'''
...
cost_tot_list = []
layer_list=[]
opZ = SparsePauliOp("Z"*n_qubit)#+"I"*(n_qubit-1))	#n_feature
