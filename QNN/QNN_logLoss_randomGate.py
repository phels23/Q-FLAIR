...
def constructCircuit_QNN(Y,normalized_Xtrain,mf,matrix_feature,theta_test_matrix,n_feature,PARAM,opZ,count,norm_Xtrain_l):
	...
	return [opt_theta_feature,opt_cost_feature,nf]
...
while count<100:
	...
	rand_collect=[]
	while True:
		i = rnd.randint(0,len(matrix_feature)-1)	#selects a random gate
		while i in rand_collect:
			i = rnd.randint(0,len(matrix_feature)-1)	#selects a random gate
		rand_collect.append(i)
	
		opt_theta_feature,opt_cost_feature,j = constructCircuit_QNN(Y_train,normalized_Xtrain,
		...
		count_rand+=1
		if count>0 and abs(cost_tot_list[-1]-min(opt_cost_feature))>1e-5:
			break
		elif count == 0:
			break
		if len(rand_collect) == len(matrix_feature):
			break
