...
def calcCost(theta):
	...
	for i in range(n_Y):
		...
		#expect = (st.expectation_value(opZ).real+1)*0.5
...
##################################################################
# replace angles by random values
print('replace angles by random values as initial points',flush=True)
rnd.seed(seed)
for i in range(len(angle)):
    angle[i] = 1-2*rnd.random()
