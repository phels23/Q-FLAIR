...
def constructCircuit_QNN(Y,normalized_Xtrain,mf,matrix_feature,theta_test_matrix,n_feature,PARAM,opZ,count,norm_Xtrain_l):
	'''
	...
	def layerParallel(ID1,mf,AA,BB,CC):
		...
		for kt in range(3):	#iterate over the three test angles len(kernel_test)
			...
			qc_new_test = circ_convertSinge(matrix_feature[mf],1,theta_test_matrix[kt],n_qubit,PARAM)	#n_feature
			qc_new_test_i = qc_new_test
			try:
				qc_new_test_i.assign_parameters({PARAM[1]: 1}, inplace = True)
			except: pass
						
			qc_unite_test_i = qc_old_i.compose(qc_new_test_i,qubits=list(range(0,n_qubit)),inplace=False)	#n_feature
	...
	if gateIsDatadependent:
		...
		opt_params = Parallel(n_jobs=n_jobs)(delayed(find_cost_rec_QNN)(AA,BB,CC,normalized_Xtrain,Y,nf,num_samples_classical) for nf in range(n_feature))
		for nf in range(n_feature):	
			opt_theta = opt_params[nf][0]
			opt_cost = opt_params[nf][1]
...
while True:
	...
	for i in range(len(matrix_feature)):
		opt_theta_feature,opt_cost_feature = constructCircuit_QNN(Y_train,normalized_Xtrain,
