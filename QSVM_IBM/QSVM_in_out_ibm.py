...
class Tee:
	...
	@property
	def encoding(self):
		# Return encoding of the first file (usually sys.__stdout__)
...
def read_data():
	...
	for line in param_open:
		...
		elif line_split[0].strip() == 'backend': backend = line_split[1].strip()
		elif line_split[0].strip() == 'use_sampler': use_sampler = bool(line_split[1].strip().lower())
	...
	# copy param file to the output directory (dirname(costFile))
	results_dir = os.path.dirname(costFile)
	os.makedirs(results_dir, exist_ok=True)
	shutil.copy(param_file, results_dir)
	...
	if args.measure or args.initialize or args.randomangle:
		...
		return [allData,seed,repetition,layers,kernelFile,gateFile,costFile,n_jobs,gateList,accOut,train_batch,shots,backend,use_sampler,nNystroem]
...
def write_model_outputs(outputs_file,acc,outputs,ID_gate):
	'''
	write the model outputs to a txt file
	'''
	outfile = open(outputs_file,"a")
	outfile.write(f'{ID_gate}\t{acc}\t')
	for i in range(len(outputs)):
		outfile.write(f'{outputs[i]}\t')
	outfile.write("\n")
	outfile.close()
