...
def determine_sine_curve(f0,fp,fm):
	...
	'''
	c = 0.5 * (fp + fm)
	b = np.arctan2(2 * (f0 - c), fp - fm)	#????????????????????????????????????????? see reference in Pennylane
...
def find_cost_angle(AA,BB,CC,diff_X_feature,Y,component,num_samples_classical):
	...
	def fn(theta):
		...
		#costs = np.mean(kernel_mat_flat, axis=1)

		#plt.plot(theta,kernel_mat_flat)
	...
	if num_xs%2==0:
		num_xs+=1
	xs = np.linspace(-1, 1, num_xs)	#exchanged upper limit 1->2pi ????
	ys = fn(xs)
	
	opt_param = xs[np.argmax(ys)]
	opt_y = np.max(ys)
	#print(opt_param,opt_y)
	#print(fn(np.array([1])),fn(np.array([2])),fn(np.array([3])))
	#print(fn(np.array([1/minDif_feature+1])),fn(np.array([1/minDif_feature+2])),
	#	fn(np.array([1/minDif_feature+3])))
	#plt.axvline(1/minDif_feature)
	#plt.plot(xs,ys)
	#plt.show()
...
def find_cost_angle_sym(AA,BB,CC,diff_X_feature,Y,component):	#solving the cost equation analythically is too expensive !!!!!!
	...
	if d1==0:
		...
		#extrema=sp.solveset(d1,th,sp.Interval(-search_range,search_range))
		print(extrema)
		th_minimum=0
		val_minimum=cost(th_minimum,AA,BB,CC,diff_X_feature,Y_sq,component)
		for ex in extrema:
